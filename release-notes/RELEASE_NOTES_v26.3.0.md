# MARS v26.3.0 Release Notes

MARS now targets .NET 10, following the ProteoWizard pwiz-sharp port to the same framework;
vendor reading and every output format are re-verified against the frozen reference cohort.
...
- **A profile non-MS spectrum was sent to the vendor centroider.** MARS decided whether to ask
  a vendor reader for a centroided spectrum by testing only whether the spectrum was profile,
  which selected diode-array and EMR traces along with real MS scans. MassLynx cannot centroid
  an absorbance-vs-wavelength trace and answers with nothing. MARS now passes the MS-level set
  the reader expects, so those spectra pass through as acquired. This is the one change in this
  release that can alter written output; see the pwiz-sharp entry below.
...
- **MARS now targets .NET 10, and building it requires a .NET 10 SDK (10.0.100 or newer).**
  ...
  **Nothing changes for anyone running a release binary.** The published artifacts are
  ...
  runtime instead of .NET 8, and still rolls forward past it.

  **Calibration output is unchanged, and this was measured rather than assumed.** The frozen
  parity guard matches on all five files of the Stellar reference cohort - 352,349 matched
  fragments, every column identical to the digests taken when the matcher was verified against
  the Python implementation - from mzML input *and* from reading the `.raw` files directly. All
  four output formats were written from a `.raw`, and the determinism tests still produce
  ...
  `dotnet --list-sdks` has to show a 10.x entry. The top-level README's install section says
  which of the three install routes needs .NET at all.
...
- **The pinned pwiz-sharp commit moved to the PR #4619 branch.**
  ...
  **This can change written output for one case:** a vendor file carrying UV/DAD or other
  non-MS spectra, converted to mzXML, mzMLb or mgf. MARS previously asked the vendor to
  centroid those too; they are now passed through as acquired. MS1 and MS2 spectra are
  unaffected, and files read as mzML never took this path at all.
